...
func newGitLabPRHandler(ctx context.Context, project string, targetRef string, extra interface{}) (PlatformHandler, error) {
	gitlabExtra, ok := extra.(GitLabExtra)
	if !ok {
	...
	serverURL := gitlabExtra.ServerURL
	...
	httpClient, graphqlClient, err := newGitLabAPIClients(ctx, gitlabExtra.Token, serverURL)
	if err != nil {
		return nil, err
	}
...
func newGitLabCommitHandler(ctx context.Context, project string, targetRef string, extra interface{}) (PlatformHandler, error) {
	gitlabExtra, ok := extra.(GitLabExtra)
	if !ok {
		return nil, errors.New("Invalid extra")
	}
...
func init() {
	...
	registerPlatformHandler("gitlab", "pull-request", newGitLabPRHandler)
	registerPlatformHandler("gitlab", "commit", newGitLabCommitHandler)
}
